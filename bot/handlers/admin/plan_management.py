...
async def start_plan_management(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        plan_list = ""
        if not plans:
            plan_list = _('messages.plan_management_no_plans_list')
        else:
            for p in plans:
                plan_list += f"- {p.name} ({p.price} {_('words.toman')})\n"
...
async def confirm_create_plan(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Saves the new plan to the database and returns to the main plan menu."""
    query = update.callback_query
    await query.answer()
...
async def cancel_plan_creation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Cancels the plan creation process and returns to the main plan menu."""
    query = update.callback_query
    await query.answer()
